...
/**
 * Chunks related fields under a compact header. Renders a real `fieldset` so
 * assistive technologies announce the group name with each control inside.
 */
    ...
    >
      <div className="flex items-start justify-between gap-3" data-slot="form-section-header">
        <div className="grid gap-0.5">
          <legend className="float-left text-control font-semibold leading-5 text-foreground">
            {title}
          </legend>
...
export type FormRowProps = ComponentPropsWithoutRef<"div"> & {
  /**
   * Equal columns for short, related fields (phone/email, city/state). Pass a
   * `grid-cols-[…]` class instead for uneven tracks.
   */
  ...
  ({ className, columns = 1, ...props }, ref) => (
    <div
      ...
      className={cn(
        ...
        columns === TWO_COLUMNS && "grid-cols-2",
        columns === THREE_COLUMNS && "grid-cols-3",
        className,
      )}
      data-slot="form-row"
      ref={ref}
