...
function registerNoDraftThenAtomicCommitTest(): void {
  void it("persists nothing before confirm, then commits every row atomically", async () => {
...
function registerAllPresentDefaultTest(): void {
  void it("defaults every untouched active enrollment to PRESENT when rows is empty", async () => {
    const scenario = await harness.seedBaseScenario();
...
function registerReconfirmRejectedTest(): void {
  void it("rejects confirming a session whose attendance is already confirmed", async () => {
    const scenario = await harness.seedBaseScenario();
    await harness.enrollStudent({
      classId: scenario.classId,
      stageId: scenario.stageId,
...
function registerCancelledSessionRejectedTest(): void {
  void it("rejects confirming a cancelled session", async () => {
    const scenario = await harness.seedBaseScenario();
    await harness.enrollStudent({
      classId: scenario.classId,
      stageId: scenario.stageId,
...
function registerOffRosterRowRejectedTest(): void {
  void it("rejects a row for an enrollment that is not on the session roster", async () => {
    const scenario = await harness.seedBaseScenario();
    await harness.enrollStudent({
      classId: scenario.classId,
      stageId: scenario.stageId,
...
function registerDuplicateRowRejectedTest(): void {
  void it("rejects duplicate enrollment rows in the same confirm", async () => {
    const scenario = await harness.seedBaseScenario();
    const ana = await harness.enrollStudent({
      classId: scenario.classId,
      stageId: scenario.stageId,
...
function registerWindowEnforcedTest(): void {
  void it("commits only in-window enrollments (enrollment window enforced)", async () => {
    const scenario = await harness.seedBaseScenario();
    const ana = await harness.enrollStudent({
      classId: scenario.classId,
      stageId: scenario.stageId,
...
function registerScopeTest(): void {
  void it("lets the owning teacher confirm, but forbids another teacher", async () => {
    const scenario = await harness.seedBaseScenario();
    await harness.enrollStudent({
      classId: scenario.classId,
      stageId: scenario.stageId,
...
function registerTeacherSameDayWriteWindowTest(): void {
  void it("blocks an owning teacher from first-confirming outside the same Sao Paulo day", async () => {
    const scenario = await harness.seedBaseScenario();
    await harness.enrollStudent({
      classId: scenario.classId,
      stageId: scenario.stageId,
