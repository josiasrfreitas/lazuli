...
function registerRosterOrderingTest(): void {
  void it("returns the active roster ordered by name, PRESENT default, nothing committed", async () => {
...
function registerWindowExclusionTest(): void {
  void it("excludes an enrollment whose window does not contain the session date", async () => {
    const scenario = await harness.seedBaseScenario();
...
function registerOtherClassExclusionTest(): void {
  void it("excludes students enrolled in a different class", async () => {
    const scenario = await harness.seedBaseScenario();
...
function registerUntakenFlagTest(): void {
  void it("flags a past unconfirmed session untaken; clears the flag after confirm", async () => {
    const scenario = await harness.seedBaseScenario();
    await harness.enrollStudent({
      classId: scenario.classId,
      stageId: scenario.stageId,
...
function registerUnknownSessionTest(): void {
  void it("rejects an unknown session with a not-found error", async () => {
...
function registerScopeTest(): void {
  void it("lets the owning teacher and admin read, but forbids another teacher", async () => {
    const scenario = await harness.seedBaseScenario();
    await harness.enrollStudent({
      classId: scenario.classId,
      stageId: scenario.stageId,
