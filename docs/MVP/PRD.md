...
- `Track`: `name`, `category`, `status` (`ACTIVE | LEGACY`), optional `portalPrefix`. `Stage`: `trackId`, `name`, `internalCode` (unique within track), `sequence` (linear order within the track).
- **Seed-only, no CRUD UI in MVP** — idempotent seed/migration derived from [discovery/course-stage-ordering.md](../discovery/course-stage-ordering.md). Legacy lines (Teens, Teenstation) seeded as `LEGACY`; Teens Connect as `ACTIVE`. Admin catalog UI is Phase 2.
...
>
> - **Entities:** **`Order`** (belongs to one **`Payer`**; covers ≥1 student via **`OrderBeneficiary`**, where Beneficiary ≡ Student) → ≥1 **`Installment`** (+ signed **`InstallmentAdjustment`** rows); **`PaymentEntry`** belongs to a **`Payer`** and is applied to installments via divisible **`PaymentAllocation`** rows that may span the payer's orders.
...
12. **Stage taxonomy & modeling** — **modeling resolved** ([D-0030](./decisions.md#d-0030-course-catalog--track-and-stage) Track → Stage; [D-0031](./decisions.md#d-0031-enrollment-is-operational-stage-placement-lives-on-pedagogicalprogress) placement on `PedagogicalProgress`). Equivalences remain unmodeled by decision. Remaining items are **school-validation only**, blocking the production seed (not the model): exact internal/Portal naming per stage, whether Speed reconnects to adult English, and whether Connect 1-4 is complete/current. Draft paths: [discovery/course-stage-ordering.md](../discovery/course-stage-ordering.md).
